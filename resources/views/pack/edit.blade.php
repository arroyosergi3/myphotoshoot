    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('pack.update', $p) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                            step="0.01" :value="old('name', $p->name)" required />
                        <x-input-error :messages="$errors->get('name')" />
                    </div>

                    <div class="mb-4">
                        <x-input-label for="description" :value="__('Descripción')" />
                        <textarea id="description" class="block mt-1 w-full rounded  border-gray-300  " name="description">{{ old('description', $p->description) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="price" class="block mt-1 w-full" type="text" name="price"
                            step="0.01" :value="old('price', $p->price)" required />
                        ...
                        <select id="photoshoot" name="photoshoot"
                            ...
                            @if (!empty($photoshoots))
                                ...
                                @foreach ($photoshoots as $ps)
                                    <option value="{{ $ps->id }}" {{ $ps->id == $p->photoshoot_id ? 'selected' : '' }}>
    {{ $ps->name }}
</option>
                    ...
                    <div x-data="{ preview: null }" class="mb-4">
                        ...
                        <!-- Input file oculto -->
                        <input id="pic" x-ref="pic" type="file" name="pic" accept="image/*"
                            class="hidden" @change="preview = URL.createObjectURL($refs.pic.files[0])" />
                        ...
                        @if ($p->img_url)
                            ...
                            <img src="{{ asset($p->img_url) }}" alt="Imagen actual"
